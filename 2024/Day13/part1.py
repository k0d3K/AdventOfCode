...
#    Updated: 2024/12/13 07:23:19 by lguerbig         ###   ########.fr        #
#                                                                              #
# **************************************************************************** #
...
def solve_equation(Ax, Ay, Bx, By, Px, Py):
    ...
    # Step 1: Check the solvability condition by calculating the GCDs
    g1 = gcd(Ax, Bx)
    g2 = gcd(Ay, By)
    ...
    # Step 2: Set up the coefficient matrix and the right-hand side vector
    A = np.array([[Ax, Bx], [Ay, By]], dtype=int)
    B = np.array([Px, Py], dtype=int)

    try:
        # Step 3: Solve the system using NumPy's linear algebra solver
        solution = np.linalg.solve(A, B)
        
        # Check if the solution is close to integers (due to numerical precision)
